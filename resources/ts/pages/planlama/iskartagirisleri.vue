<template>
  <VCard>
    ...
    <DxDataGrid id="gridContainer" ref="dataGridRef" @content-ready="onContentReady" :data-source="gridData"
      :show-borders="true" key-expr="ID" :show-column-lines="true" :show-row-lines="false"
      :row-alternation-enabled="false" :allow-column-reordering="true" @row-removed="onRowRemoved"
      ...
      <DxColumn data-field="ISEMRIID" data-type="number" caption="İŞEMRİ ID" :visible="true" :width="80" />
...
var mesaj = 'Iskarta Girişleri'
document.title = "OFT - Iskarta Girişleri";
...
const onRowRemoved = (e: any) => {
  axios.put(`/api/iskartasil/${e.key}`, e.data, {
    headers: {
      'userID': userData.value.id,
    }
  })
...
const onExporting = (e: DxDataGridTypes.ExportingEvent) => {
  ...
  const worksheet = workbook.addWorksheet("IskartaGirisleri");
